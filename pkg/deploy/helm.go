...
func helmDeployCommand(
	...
	if workloadType != "deployment" && workloadType != "statefulset" && workloadType != "job" && workloadType != "cronjob" {
		return command.Error(
			fmt.Errorf("Workload type must be either deployment, statefulset, cronjob or job, got '%s'.", workloadType),
		)
	}
